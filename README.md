...
- 加载单个或多个 PAK，也可以选择游戏目录后自动扫描目录内的 PAK。
- 选择路径 list 后浏览、搜索和定位资源。
- 支持 TEX 贴图预览，并可导出 PNG / DDS。
- 支持 Mesh 模型、骨架、材质、贴图和动画预览。
- 模型默认先快速加载几何；需要材质贴图时可在预览区手动加载贴图。
- 支持多个模型分件叠加或同骨骼合并预览。
- 支持播放 MotionList 动画，进度条手动拖动后会自动暂停。
- 动画时间显示同时包含秒数、当前帧、总帧数和源帧率。
- 支持导出当前预览模型 FBX，以及当前动画或全部动画 FBX。
- 动画导出按源动画速度和有效帧范围输出，避免尾部重复静止帧。
- 导出文件夹默认位于程序 EXE 同级目录，可在主界面和设置中直接打开。
- 首次进入会弹出环境窗口；之后也可以从菜单栏重新打开环境窗口。

## 当前支持状态

| 游戏 / 能力 | 状态 |
| --- | --- |
| 《鬼武者：剑之道》DEMO | 已验证 PAK、贴图、模型、骨架、动画预览与 FBX 导出工作流 |
| 《街头霸王 6》 | 已验证 PAK、list、模型基础材质与贴图读取；动画仍属于实验性支持 |
...
4. 在 list 管理器中选择或下载对应游戏的路径 list。
5. 拖入 PAK，或选择游戏目录后扫描并加载 PAK。
6. 双击资源进行预览，使用右键菜单或右侧导出面板提取和导出。
...
默认情况下，可写数据会放在程序 EXE 同级目录，便于打包和迁移：
