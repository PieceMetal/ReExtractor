...
- 修复《生化危机 2》扫描遇到空 PAK 占位文件时加载失败的问题，并提示路径列表未匹配任何资源的情况。
- 为 RE RT 动画增加骨架兼容性检查，阻止分离面部、武器动作错误套用到身体模型导致拉伸。
- 补齐 RE RT 骨骼缩放轨道的预览与动画导出，保留原始动画位置及根运动。
- 修正动作切换失败和异步加载时的选择状态。详见 [版本说明与验证范围](docs/RELEASE_NOTES_v1.4.4.md)。

## v1.4.1 更新

- 修复《鬼武者：剑之道》过场面部动画的脸部塌缩、眼球与牙齿错位，同步修正预览和动画导出。详见 [版本说明](docs/RELEASE_NOTES_v1.4.1.md)。

## v1.4.0 更新

- 新增工具内反馈侧栏，支持粘贴图片、选择图片和附带日志。
- 优化评论日期、附件布局，以及反馈入口的拖动和侧栏动画。
- 配套反馈后台支持管理评论、设备屏蔽、修复队列和修复记录。
- 公共评论与后台同步，归档和修复保留评论，隐藏及清档结果同步到工具。

## v1.3.22 更新

- 修复怪物猎人崛起黑蚀龙、天廻龙加载动画后因独立根骨骼导致导出失败的问题。详见 [版本说明](docs/RELEASE_NOTES_v1.3.22.md)。
...
- 模型导出名称采用预设名或资源名，加 LOD 档位，去掉合并数量后缀。
- 保存完整 Blender 转换日志，明确区分脚本异常与未收到成功标记。详见 [版本说明](docs/RELEASE_NOTES_v1.3.20.md)。

## v1.3.19 更新

- 新增 LOD 切换，默认 LOD0 最高精度，预览及模型导出使用所选档位。
- 修复此前固定读取 LOD1 导致模型精度偏低的问题。详见 [版本说明](docs/RELEASE_NOTES_v1.3.19.md)。

## v1.3.18 更新

- 修复多分件角色合并导出遇到重复骨骼名时失败的问题。
- 在线列表增加 GitHub 接口不可用时的网页目录回退。
- 窗口及顶部版本号自动读取实际程序版本。详见 [版本说明](docs/RELEASE_NOTES_v1.3.18.md)。

## v1.3.17 更新

- 预览模型导出等待关联贴图全部处理完后再恢复按钮，避免提前误判完成。
- 关联贴图导出创建 textures 文件夹，并记录逐张结果、保存位置和失败原因；缺失的 MDF 贴图引用会明确提示。
- 静御前普通模型实测导出 21 张 PNG，包含 5 张 NRRO 贴图。详见 [版本说明](docs/RELEASE_NOTES_v1.3.17.md)。

## v1.3.16 更新

- 材质查找新增候选发现和完整材质名校验，支持更多特殊命名及同目录共用材质，避免按列表顺序错贴。
- 多个有效候选会提示选择；预览、FBX 和关联 PNG 共用选择，右键“加载贴图”可重选。
- 鬼武者角色目录 549 个分件的材质名检查：546 个自动匹配、3 个需要选择；原有 529 个匹配结果保持不变。详见 [版本说明](docs/RELEASE_NOTES_v1.3.16.md)。

## v1.3.15 更新

- 修复《鬼武者：剑之道》静御前的材质匹配遗漏，恢复模型贴图加载。
- 修复静御前头部表情图集导致的重复五官；原始图集导出保持完整分辨率。
- 批量模型导出补齐 MDF 关联贴图，法线、遮罩等 PNG 保存于 `textures` 文件夹，优先读取高清 streaming 资源。
- 已实测静御前六个分件，共导出 72 张 PNG；两张三维特效纹理仍不支持解码。详见 [版本说明](docs/RELEASE_NOTES_v1.3.15.md)。

## v1.3.14 更新

- 修复《街头霸王 6》角色皮肤发黑、眉毛和胡子发白，按角色服装读取默认配色。
- 修复衣服染色区域丢失、隆的布料散线显示灰白，以及合并后头发与眉毛颜色串用的问题。
- 修正毛发透明贴图采样；省略通用预览器尚不支持的眼部泪膜覆盖层，避免眼下白边。
- 隆和肯的完整角色已完成材质对比检查及用户试用确认。详见 [版本说明](docs/RELEASE_NOTES_v1.3.14.md)。

## v1.3.13 更新

- 修复《鬼武者》正式版的 MDF 材质读取，以及路径列表未收录 streaming 资源时只能读取低分辨率贴图的问题。
- 修复角色分件预览正常、合并模型与动画却无法导出的问题：保留各分件的绑定数据，动画轨道按骨骼名称匹配，并保留最多 8 个蒙皮权重。
- 新增右键“批量导出选中模型 FBX”；右键加入合并器和“添加选中”均支持多选。
- 加载预设默认打开当前游戏的预设目录，并新增“打开预设目录”按钮。
- 列表管理器自动获取 Ekey 的 Projects 目录，支持缓存、“新增”和“可更新”提示，保留 Eigeen 来源；下载和切换列表仍由用户操作。
- 已对鬼武者正式版五个分件和 `mot_860` 完成 FBX 导出、Blender 回读及绑定一致性检查。详见 [版本说明](docs/RELEASE_NOTES_v1.3.13.md)。
...
- 修复《怪物猎人：崛起 / 曙光》NPC 由脸部或头发分件误作为主骨架驱动，导致播放动画时身体断开、头部或四肢飞散的问题。
- 主骨架现在按实际蒙皮骨数选择，不再被动画辅助骨数量干扰；身体、脸部和头发使用同一套身体姿势。
- 撤回 v1.3.8 / v1.3.9 测试包中错误的 NPC 旋转转换，恢复 RE Engine MOT 原始旋转数据。
- 修复 `npc615_00` 等含重复辅助骨名的模型在补充动画骨时直接崩溃的问题。
- 已覆盖 list 中 23 套完整 NPC 与 394 条动作，并对 `npc002_00_00` 的 `mot_9` 第 227/480 帧做定点回归检查。
